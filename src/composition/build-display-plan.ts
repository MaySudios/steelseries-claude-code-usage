...
import { type RenderPlan } from '../application/plugin-service.js';
...
export function buildDisplayPlan(config: Config): BuiltPlan {
  ...
  const maxLines = Math.max(1, ...config.oled.screens.map((screen) => screen.lines.length));
  ...
  const renderPlan: RenderPlan = {
    ...
    screens: config.oled.screens.map((screen) => ({ lines: screen.lines })),
    rotateSeconds: config.oled.rotateSeconds,
...
function buildHandler(binding: KeyBindingConfig, hidKeys: number[]): GameSenseHandler {
  switch (binding.type) {
    ...
    case 'pulse':
      return pulseHandler(hidKeys, rgbFromHex(binding.color), binding.minHz, binding.maxHz);
  }
}
